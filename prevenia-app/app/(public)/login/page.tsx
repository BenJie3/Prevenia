...
import toast from "react-hot-toast"; // 👈 Alertas modernas (Toasts)
import { Eye, EyeOff } from "lucide-react"; // 👈 Iconos del ojito
...
export default function AuthPage() {
  ...
  // 🛡️ Consentimiento legal para el registro tradicional
  const [termsAccepted, setTermsAccepted] = useState(false);
  const [showPassword, setShowPassword] = useState(false); // 👈 Estado del Ojito
  const [isLoading, setIsLoading] = useState(false);

  // EL CADENERO INVERSO: Si ya hay sesión, te expulsa al Dashboard
  useEffect(() => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.success("¡Cuenta creada! Por favor inicia sesión.", { id: toastId, duration: 4000 });
        setName(""); setPassword(""); setTermsAccepted(false); setIsLogin(true); 
      }
    } catch (error) {
          ...
          {isLogin ? (
            <motion.div key="login" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.3 }}>
              <form className="space-y-5" onSubmit={handleCredentialsLogin}>
                ...
                {/* 🛡️ Consentimiento Implícito (Debajo del botón tradicional de Acceder) */}
            ...
            <motion.div key="register" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }}>
              <form className="space-y-5" onSubmit={handleRegister}>
                ...
                <button 
                  ...
                  className="w-full bg-[#6B8E7D] text-white font-inter font-light py-4 rounded-full hover:bg-[#2C332B] transition mt-8 tracking-wide shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                >
